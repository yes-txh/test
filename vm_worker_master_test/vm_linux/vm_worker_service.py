...
# 导入thrift定义的模块
from gen_vm_worker.vm_worker import VMWorker  # service
from gen_vm_worker.vm_worker.ttypes import *  # struct
from gen_executor.worker.ttypes import * # worker struct
...
class VMWorkerHandler:
	...
	app_install_worker = threading.Thread(target = vm_worker_app.AppInstallWorker, args =(app_info.id, app_info.install_dir))
	...
	app_run_worker = threading.Thread(target = vm_worker_app.AppRunWorker, args =(app_info.id, app_info.name, app_info.source, app_info.exe, app_info.argument, app_info.run_type, app_info.interval)) 
